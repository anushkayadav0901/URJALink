...
def calculate_solar_potential(
    roof_data: dict, solar_data: dict, obstacles: dict
) -> dict:
    """Calculate complete solar potential with deterministic modifiers."""
    total_panels = sum(
        segment["panel_capacity"] for segment in roof_data["roof_segments"]
    )
    ...
    energy_offset_percent = min((annual_kwh / TYPICAL_HOME_CONSUMPTION_KWH) * 100, 100)
...
def calculate_solar_score(
    roof_data: dict, solar_data: dict, solar_potential: dict, obstacles: dict
) -> int:
    ...
    orientation_factor = _orientation_factor(roof_data)
    roof_score = min(
        (usable_area / IDEAL_USABLE_AREA_SQFT) * orientation_factor * 100, 100
    )

    peak_sun_hours = solar_data["peak_sun_hours_daily"]
    irradiance_score = min((peak_sun_hours / SCORE_REFERENCE_PEAK_SUN_HOURS) * 100, 100)

    system_size_kw = solar_potential["system_size_kw"]
    size_score = min((system_size_kw / SCORE_REFERENCE_SYSTEM_SIZE_KW) * 100, 100)

    capacity_factor = solar_potential["capacity_factor"]
    cf_score = min((capacity_factor / SCORE_REFERENCE_CAPACITY_FACTOR) * 100, 100)
