...
class EquityScoreCalculator:
    ...
    def calculate_score(metrics: EquityMetrics) -> EquityScoreBreakdown:
        ...
        income_score = min(
            40.0, 
            (metrics.median_income / EquityScoreCalculator.INCOME_THRESHOLD) * 40
        )
        ...
        burden_score = max(
            0.0, 
            20 - (metrics.energy_burden / EquityScoreCalculator.ENERGY_BURDEN_THRESHOLD) * 20
        )
        ...
        adoption_score = min(
            10.0, 
            (adoption_rate / EquityScoreCalculator.SOLAR_ADOPTION_THRESHOLD) * 10
        )
