...
from models.requests import AnalyzeRequest, AgentsRequest, SummaryRequest, BillComparisonRequest
...
from business_logic.cv_roof_analysis import analyze_roof_with_cv, get_estimated_roof_data_fallback
from business_logic.roof_analysis import get_roof_data_from_polygon
from business_logic.solar_calculations import calculate_solar_potential, calculate_solar_score, get_solar_score_breakdown
...
from models.equity_models import EquityMetrics, EquityAnalysisResponse, BlockGroupEquityData
...
async def analyze(request: AnalyzeRequest):
    ...
    try:
        ...
        neighborhood_data = [
            BlockGroupEquityData(**bg) for bg in all_equity_data
        ]
        ...
        return AnalysisResponse(
            ...
            roof_analysis=RoofAnalysis(
                ...
                roof_segments=[
                    RoofSegment(
                        ...
                        polygon=RoofPolygon(**segment['polygon']) if segment.get('polygon') else None
            ...
            solar_data=SolarData(
                ...
                seasonal_variation=SeasonalVariation(**solar_data_result['seasonal_variation'])
            ),
