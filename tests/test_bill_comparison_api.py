...
async def test_bill_comparison_endpoint_success(client, sample_pdf, monkeypatch):
    ...
    monkeypatch.setattr("business_logic.bill_extractor.settings.GEMINI_API_KEY", "test-key")
    
    def mock_extract_monthly_bill(bill_text: str, **kwargs):
        return 150.50
    
    with patch("api.routes.extract_monthly_bill", side_effect=mock_extract_monthly_bill):
...
def test_bill_comparison_endpoint_invalid_negative_values(client, sample_pdf, monkeypatch):
...
@pytest.mark.asyncio
async def test_bill_comparison_endpoint_bill_extraction_error(client, sample_pdf, monkeypatch):
...
@pytest.mark.asyncio
async def test_bill_comparison_endpoint_zero_bill_amount(client, sample_pdf, monkeypatch):
