...
if importlib.util.find_spec("pydantic_settings") is None:
    pytest.skip(
        "pydantic_settings is required for bill comparison tests.",
...
async def test_extract_monthly_bill_success(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            status_code=200,
            json={"candidates": [{"content": {"parts": [{"text": "150.50"}]}}]},
        )
...
async def test_extract_monthly_bill_no_number(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            status_code=200,
            json={
                "candidates": [{"content": {"parts": [{"text": "No amount found"}]}}]
            },
        )
...
def test_calculate_bill_comparison():
    ...
    result = calculate_bill_comparison(
        monthly_bill=200.0, monthly_savings=50.0, payback_period_years=10.5
    )
...
def test_calculate_bill_comparison_zero_savings():
    ...
    result = calculate_bill_comparison(
        monthly_bill=200.0, monthly_savings=0.0, payback_period_years=25.0
    )
...
def test_calculate_bill_comparison_rounding():
    ...
    result = calculate_bill_comparison(
        monthly_bill=199.999, monthly_savings=49.999, payback_period_years=10.99
    )
...
def test_calculate_bill_comparison_excess_generation():
    ...
    result = calculate_bill_comparison(
        monthly_bill=52.04, monthly_savings=279.0, payback_period_years=24.0
    )
