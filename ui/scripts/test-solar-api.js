  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        console.log('   Enable it at: https://console.cloud.google.com/apis/library/solar.googleapis.com\n');
      }
    ...
    console.log(`   Center: ${data.center?.latitude}, ${data.center?.longitude}`);
    console.log(`   Image date: ${data.imageryDate?.year}-${data.imageryDate?.month}-${data.imageryDate?.day}`);
    ...
    if (data.solarPotential) {
      ...
      console.log(`     Roof segments: ${potential.roofSegmentStats?.length || 0}`);
      
      if (potential.roofSegmentStats && potential.roofSegmentStats.length > 0) {
        console.log(`\n   📐 Roof Segments:`);
        potential.roofSegmentStats.forEach((segment, index) => {
          console.log(`     Segment ${index + 1}:`);
          console.log(`       Center: ${segment.center.latitude}, ${segment.center.longitude}`);
          console.log(`       SW: ${segment.boundingBox.sw.latitude}, ${segment.boundingBox.sw.longitude}`);
          console.log(`       NE: ${segment.boundingBox.ne.latitude}, ${segment.boundingBox.ne.longitude}`);
          if (segment.stats) {
            console.log(`       Area: ${segment.stats.areaMeters2.toFixed(2)} m²`);
          }
        });
      }
    }
