...
async function diagnosticTest() {
  console.log('\n');
  log('╔════════════════════════════════════════════════════════════╗', colors.cyan);
  log('║          Diagnostic Test for Backend Developer           ║', colors.cyan);
  log('╚════════════════════════════════════════════════════════════╝', colors.cyan);
  console.log('\n');
  ...
  try {
    ...
    log(`\n✓ Status Code: ${response1.status} ${response1.statusText}`, colors.green);
    log(`✓ Content-Type: ${response1.headers.get('content-type')}`, colors.green);
    ...
    log(`\n✓ Response Body Length: ${fullText.length} characters`, colors.green);
    ...
    while (true) {
      ...
      const dataLinesInChunk = lines.filter(line => line.startsWith('data: ')).length;
    ...
    if (chunkCount === 0) {
      ...
      log('\n⚠️  PROBLEM: Received data but no "data: " SSE format lines', colors.yellow);
      ...
      log(`\n📝 Extracted Content (${actualContent.length} chars):`, colors.cyan);
      log('─'.repeat(60), colors.gray);
