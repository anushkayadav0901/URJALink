  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white">
      ...
      <p className="text-sm text-white/70 mb-4">
        Upload your electricity bill (PDF) to see detailed before and after savings
      </p>
      ...
      {!comparisonResult ? (
        <div className="space-y-4">
          ...
          >
            ...
            />
            <Upload className={`h-10 w-10 mb-2 ${selectedFile ? "text-emerald-300" : "text-white/40"}`} />
            {selectedFile ? (
              <div className="text-center">
                <p className="text-sm font-medium text-emerald-300">{selectedFile.name}</p>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-white/60 mt-1">Max file size: 10MB</p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 p-3">
            ...
            <p className="text-sm text-emerald-300">Bill analyzed successfully!</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h5 className="text-sm font-medium text-white/80 uppercase tracking-wider">Current Bill</h5>
                </div>
              ...
              <div className="rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/10 to-transparent p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h5 className="text-sm font-medium text-emerald-300 uppercase tracking-wider">After Solar</h5>
                </div>
                <p className="text-3xl font-bold text-emerald-300">
                  ${Math.abs(comparisonResult.new_monthly_bill).toFixed(2)}
                  {comparisonResult.new_monthly_bill < 0 && <span className="text-lg"> credit</span>}
                </p>
                <p className="text-xs text-white/60 mt-1">per month</p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-emerald-500/30 bg-gradient-to-r from-emerald-400/20 to-transparent p-5 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <h5 className="text-sm font-medium text-white/80 uppercase tracking-wider">Annual Savings</h5>
              </div>
              <p className="text-4xl font-bold text-emerald-300">
                ${((comparisonResult.current_monthly_bill - comparisonResult.new_monthly_bill) * 12).toFixed(2)}
              </p>
              <p className="text-sm text-white/70 mt-2">
                Monthly difference: ${(comparisonResult.current_monthly_bill - comparisonResult.new_monthly_bill).toFixed(2)}
              </p>
            </div>
          </div>
