...
export const EquityActionsPanel = ({ equityData, userAddress }: EquityActionsPanelProps) => {
  const { toast } = useToast();

  const handleGenerateReport = () => {
    toast({
      title: "Generating Equity Report",
      description: "This feature will create a comprehensive PDF including detailed score breakdown, neighborhood comparison, and policy recommendations.",
    });
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-white/70 mb-6 leading-relaxed">
        {isSolarDesert ? (
          <>
            Your area faces barriers to solar access. Use these tools to advocate 
            for equitable energy programs and connect with assistance organizations.
          </>
        ) : (
          <>
            Help make solar energy accessible to all communities in {equityData.area_description}. 
            Share insights and advocate for energy equity.
          </>
        )}
      </p>
      ...
      {equityData.solar_deserts_count > 0 && (
        ...
        >
          ...
          <p className="text-sm text-white/70 leading-relaxed">
            These neighborhoods have high solar potential but face income, ownership, 
            or program access barriers. Targeted interventions could unlock significant 
            clean energy benefits for underserved communities.
          </p>
        </motion.div>
      )}
    </motion.div>
...
const ActionButton = ({ icon, label, description, onClick, primary }: ActionButtonProps) => {
  return (
    <Button
      onClick={onClick}
      variant="ghost"
    ...
    >
      <div className="flex items-start gap-4 w-full">
        <div className={`p-2 rounded-xl ${
          primary ? 'bg-blue-400/20 text-blue-200' : 'bg-white/10 text-white/80'
        }`}>
          {icon}
        </div>
