  ...
  for (let lat = bounds.south; lat <= bounds.north; lat += latStep) {
    for (let lng = bounds.west; lng <= bounds.east; lng += lngStep) {
      ...
      if (nearest && getDistanceKm(lat, lng, nearest.lat, nearest.lng) <= 1.5) {
        ...
        const interpolatedScore = Math.max(
          0,
          Math.min(100, nearest.equity_score + scoreVariance),
        );
...
const getGradientColor = (
  ...
  const factor =
    (score - lowerStop.score) / (upperStop.score - lowerStop.score);
...
export const EquityHeatmap: React.FC<EquityHeatmapProps> = ({
  ...
  useEffect(() => {
    ...
    console.log(
      "✅ Generated",
      allPoints.length,
      "equity points for comprehensive coverage",
    );
  ...
  const onLoad = React.useCallback(
    function callback(map: google.maps.Map) {
      console.log(
        "🗺️ Sophisticated equity heatmap loaded with",
        dataPoints.length,
        "data points",
      );
      ...
      >
        ...
        {denseGridPoints.map((point, index) => {
          ...
          return (
            <Circle
              key={
                point.isInterpolated ? `grid-${index}` : point.block_group_id
              }
            ...
            <div className="p-4 bg-white rounded-lg shadow-xl border border-gray-100">
              ...
              >
                <span className="text-sm font-semibold text-gray-800">
                  {selectedPoint.equity_score >= 65
                    ? "✅ High Equity Area"
                    : selectedPoint.equity_score >= 35
                      ? "⚠️ Moderate Equity"
                      : "🚨 Solar Desert"}
                </span>
                <div className="text-xs text-gray-600 mt-1">
                  Score reflects income, ownership, energy burden, and solar
                  adoption
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="text-center p-2 bg-gray-50 rounded">
                  ...
                  <div className="text-xs text-gray-600">
                    Median household income in the area (USD)
                  </div>
                </div>
                <div className="text-center p-2 bg-gray-50 rounded">
                  <div className="text-lg font-bold text-gray-900">
                  ...
                  <div className="text-xs text-gray-600">
                    Number of solar panel installation providers within service
                    area
                  </div>
                </div>
                <div className="text-center p-2 bg-gray-50 rounded">
                  ...
                  <div className="text-xs text-gray-600">
                    Average percentage of household income spent on energy costs
                  </div>
                </div>
                <div className="text-center p-2 bg-gray-50 rounded">
                  <div className="text-lg font-bold text-gray-900">
                  ...
                  <div className="text-xs text-gray-600">
                    Average number of solar panel systems installed in the area
                  </div>
                </div>
              </div>
      ...
      <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-xl border border-white/20">
        ...
        <div className="mt-3 pt-2 border-t border-gray-200">
          <p className="text-xs text-gray-600">
            Click areas for detailed analysis
          </p>
        </div>
      </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-bold text-gray-900">
              {denseGridPoints.length.toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between items-center">
