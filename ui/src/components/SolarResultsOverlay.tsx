...
import { X, Zap, DollarSign, Leaf, Sun, TrendingUp, Home, Battery, Calendar, Award, MapPin, Gift, Sparkles, Info, Globe } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { generateNarrativeSummary, EquityAnalysisResponse, fetchInstallers, fetchIncentives } from "@/lib/URJALINK-api";
...
export const SolarResultsOverlay = ({ stats, address, onClose, isVisible, latitude, longitude }: SolarResultsOverlayProps) => {
  const [activeTab, setActiveTab] = useState<'analysis' | 'actions' | 'equity'>('analysis');
  ...
  useEffect(() => {
    if (installersContent) {
      ...
      console.log("Newline count:", (installersContent.match(/\n/g) || []).length);
      console.groupEnd();
    }
  }, [installersContent]);
  ...
  useEffect(() => {
    if (incentivesContent) {
      ...
      console.log("Newline count:", (incentivesContent.match(/\n/g) || []).length);
      console.groupEnd();
    }
  }, [incentivesContent]);
  ...
  const extractAddressComponents = async (lat: number, lng: number) => {
    try {
      ...
      if (result.results[0]) {
        ...
        const state = components.find(c => c.types.includes("administrative_area_level_1"))?.short_name || "CA";
        const zipCode = components.find(c => c.types.includes("postal_code"))?.short_name || "00000";
  ...
  const buildSolarPotentialPayload = (solarStats: SolarStats) => {
    ...
    if (solarPotential) {
      return {
        ...solarPotential,
        system_size_kw: solarPotential.system_size_kw ?? solarStats.systemSizeKw,
        annual_generation_kwh: solarPotential.annual_generation_kwh ?? solarStats.yearlyEnergyKwh,
        daily_generation_kwh: solarPotential.daily_generation_kwh ?? solarStats.dailyEnergyKwh,
        capacity_factor: solarPotential.capacity_factor ?? Number((solarStats.yearlyEnergyKwh / (solarStats.systemSizeKw * 8760 || 1)).toFixed(2)),
        energy_offset_percent: solarPotential.energy_offset_percent ?? Math.min(100, Math.round((solarStats.yearlyEnergyKwh / (solarStats.systemSizeKw * 1500 || 1)) * 100)),
        co2_offset_tons_yearly: solarPotential.co2_offset_tons_yearly ?? Number((solarStats.carbonOffset / 1000).toFixed(2)),
        co2_offset_tons_25year: solarPotential.co2_offset_tons_25year ?? Number(((solarStats.carbonOffset / 1000) * 25).toFixed(2)),
        equivalent_trees_planted: solarPotential.equivalent_trees_planted ?? solarStats.treesEquivalent,
      };
    }

    const yearlyHours = solarStats.systemSizeKw * 8760 || 1;
    const capacityFactor = Number((solarStats.yearlyEnergyKwh / yearlyHours).toFixed(2));
    ...
    return {
      ...
      energy_offset_percent: Math.min(100, Math.round((solarStats.yearlyEnergyKwh / (solarStats.systemSizeKw * 1500 || 1)) * 100)),
  ...
  useEffect(() => {
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        const response = await generateNarrativeSummary(payload, { signal: controller.signal });
        if (!isMounted) return;

        setSummaryMarkdown(response.summary_markdown);
        setSummaryModel(response.model_name);
        setSummaryGeneratedAt(response.generated_at);
      } catch (error) {
        if (!isMounted || (error instanceof DOMException && error.name === "AbortError")) {
          return;
        }
        console.error("❌ Narrative summary error:", error);
        setSummaryError(error instanceof Error ? error.message : "Failed to generate narrative summary");
      } finally {
        if (isMounted) {
  ...
  const handleStreamBoth = async () => {
    ...
    const fallbackAddress = address || stats?.location?.address || "Selected location";

    if (!stats?.analysisId || !fallbackLat || !fallbackLng) {
      toast({
        title: "Missing data",
        description: "Unable to fetch results without complete analysis details",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { state, zipCode } = await extractAddressComponents(fallbackLat, fallbackLng);
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to load installers and incentives",
        variant: "destructive",
      });
    } finally {
  ...
  const solarInsightCopy =
    solarScore >= 85 ? "Exceptional potential" : solarScore >= 70 ? "Great opportunity" : "Solid opportunity";
                  ...
                  <div className="relative flex flex-col gap-6 pr-6 md:pr-12">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-[0.5em] text-white/70">Solar Analysis</p>
                        <h2 className="text-3xl font-semibold leading-tight sm:text-4xl">Solar Analysis Results</h2>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2 text-[0.7rem] font-semibold uppercase tracking-[0.4em] text-white/80">
                          <span className="rounded-full border border-white/30 bg-white/15 px-3 py-1 backdrop-blur-xl">
                            {stats.analysisType === "user-defined" ? "User-Defined Layout" : "AI-Detected Layout"}
                          </span>
                          <span className="rounded-full border border-white/30 bg-white/15 px-3 py-1 backdrop-blur-xl">
                      ...
                      <div className="rounded-3xl border border-white/30 bg-white/10 px-6 py-5 text-right shadow-inner backdrop-blur-2xl sm:min-w-[220px]">
                        <div className="flex items-center justify-end gap-2">
                          <p className="text-xs font-semibold uppercase tracking-[0.4em] text-white/70">Solar Score</p>
                          <button
                        ...
                        <div className="flex items-baseline justify-end gap-1">
                          <span className="text-5xl font-bold leading-none">{solarScore}</span>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="rounded-2xl border border-white/25 bg-white/10 p-4 shadow-inner backdrop-blur-2xl">
                        <p className="text-xs font-semibold uppercase tracking-[0.4em] text-white/70">Confidence</p>
                        <p className="text-sm text-white/80">{stats.confidence}% certainty based on AI scan</p>
                      ...
                      <div className="rounded-2xl border border-white/25 bg-white/10 p-4 shadow-inner backdrop-blur-2xl">
                        <p className="text-xs font-semibold uppercase tracking-[0.4em] text-white/70">System Snapshot</p>
                        <div className="mt-3 grid grid-cols-2 gap-3 text-sm text-white">
                          <div>
                            <p className="text-white/70">System Size</p>
                            <p className="text-xl font-semibold">{stats.systemSizeKw.toFixed(1)} kW</p>
                          </div>
                          <div>
                            <p className="text-white/70">Daily Output</p>
                            <p className="text-xl font-semibold">{formatNumber(Math.round(stats.dailyEnergyKwh))} kWh</p>
                          </div>
                          <div>
                            <p className="text-white/70">Usable Roof</p>
                            <p className="text-xl font-semibold">{formatNumber(Math.round(stats.usableAreaSqft))} sqft</p>
                          </div>
                          <div>
                            <p className="text-white/70">Max Panels</p>
                            <p className="text-xl font-semibold">{formatNumber(stats.maxPanels)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-center">
                    <div className="inline-flex w-full max-w-2xl items-center gap-1 rounded-full border border-white/10 bg-white/5 p-1 shadow-inner backdrop-blur-xl">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (installersContent === "" && incentivesContent === "") {
                            handleStreamBoth();
                          } else {
                            setActiveTab('actions');
                          }
                      ...
                      >
                        ...
                        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-[0.4em] text-white/60">Gemini Narrative</p>
                            ...
                            <div className="text-xs text-white/60">
                              {summaryModel ? `Model • ${summaryModel}` : "Model warming up"}
                              {summaryGeneratedAt && (
                                <> · {new Date(summaryGeneratedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</>
                              )}
                            </div>
                          </div>
                          <div className="prose prose-sm prose-invert mt-4 min-h-[140px] space-y-3 rounded-2xl border border-white/5 bg-black/20 p-4 prose-headings:text-white prose-a:text-white break-words">
                            {isSummaryLoading ? (
                              ...
                              <p className="text-white/70">Summary not available yet. Hang tight for Gemini to respond.</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                            <div className="flex items-center justify-between gap-3">
                              <div>
                                ...
                                <p className="text-sm text-white/70">Projected performance across 25 years</p>
                              </div>
                            ...
                            <div className="mt-6 grid gap-4 sm:grid-cols-2">
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">System Cost</p>
                                <p className="text-2xl font-semibold">{formatCurrency(stats.systemCost)}</p>
                              </div>
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">Annual Savings</p>
                                <p className="text-2xl font-semibold">{formatCurrency(stats.annualSavings)}</p>
                              </div>
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">Payback</p>
                                <p className="text-2xl font-semibold">{stats.paybackYears} years</p>
                              </div>
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">ROI Horizon</p>
                                <p className="text-2xl font-semibold">{stats.roi}%</p>
                              </div>
                            </div>
                            <div className="mt-6 rounded-2xl border border-white/10 bg-gradient-to-r from-emerald-400/30 to-transparent p-5 text-center shadow-inner">
                              <p className="text-sm uppercase tracking-[0.4em] text-white/80">Total 25-Year Value</p>
                              <p className="mt-2 text-4xl font-bold">{formatCurrency(stats.annualSavings * 25)}</p>
                              <p className="text-sm text-white/70">Includes incentives + projected utility escalation</p>
                            </div>
                            ...
                            <div className="mt-6">
                              <BillComparisonUpload
                                ...
                                monthlySavings={Math.round(stats.annualSavings / 12)}
                              />
                            </div>
                          </div>
                          ...
                          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                            ...
                            <p className="text-sm text-white/70">Meaningful offsets for your community footprint</p>
                            <div className="mt-6 space-y-5">
                              <div className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                                ...
                                <div>
                                  <p className="text-xs uppercase tracking-[0.4em] text-white/60">CO₂ offset yearly</p>
                                  <p className="text-3xl font-bold">
                                    {formatNumber(Math.round(stats.carbonOffset / 1000))} <span className="text-lg">tons</span>
                                  </p>
                                </div>
                              </div>
                              <div className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                                ...
                                <div>
                                  <p className="text-xs uppercase tracking-[0.4em] text-white/60">Trees Equivalent</p>
                                  <p className="text-3xl font-bold">
                                    {formatNumber(stats.treesEquivalent)} <span className="text-lg">trees/year</span>
                                  </p>
                                </div>
                              </div>
                            </div>
                            <div className="mt-6 grid gap-4 sm:grid-cols-2">
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">Carbon Offset</p>
                                <p className="text-xl font-semibold">{formatNumber(stats.carbonOffset)} kg lifetime</p>
                              </div>
                              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                                <p className="text-xs uppercase tracking-[0.4em] text-white/60">Trees Planting Power</p>
                                <p className="text-xl font-semibold">{formatNumber(stats.treesEquivalent * 10)} saplings</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      >
                        ...
                        <div className="text-center mb-6">
                          ...
                          <p className="text-sm text-white/70 max-w-2xl mx-auto leading-relaxed">
                            Analyzing disparities in solar access across income, ownership, and energy burden. 
                            This score helps identify communities that need targeted support to access clean energy.
                          </p>
                        </div>
                        ...
                        {stats?.fullResponse?.equity_analysis ? (
                          <>
                            {/* User's Equity Score */}
                            <EquityScoreCard score={stats.fullResponse.equity_analysis.user_address_score} />
                            ...
                            <div className="space-y-4">
                              <div>
                                <h3 className="text-xl font-semibold text-white mb-2 flex items-center gap-2">
                                  ...
                                  Solar Equity in {stats.fullResponse.equity_analysis.area_description}
                                </h3>
                                <p className="text-sm text-white/70 mb-4">
                                  Click on any area to see detailed equity metrics. Red areas ("Solar Deserts") 
                                  have high solar potential but face systemic barriers.
                                </p>
                              </div>
                              <EquityHeatmap 
                                dataPoints={stats.fullResponse.equity_analysis.neighborhood_data}
                                center={{
                                  lat: latitude || stats.fullResponse.location.latitude,
                                  lng: longitude || stats.fullResponse.location.longitude
                                }}
                                onBlockGroupClick={(blockGroup) => {
                                  console.log('Selected block group:', blockGroup);
                                }}
                              />
                            </div>
                            ...
                            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
                              ...
                              <p className="text-sm text-white/70 leading-relaxed">
                                The URJALINK Equity Score (0-100) measures how accessible solar energy is for 
                                a community. It considers income levels (can people afford solar loans?), 
                                ownership rates (do residents control their roofs?), energy burden (how much 
                                of income goes to utilities?), and existing solar adoption (are there proven 
                                programs?). A low score doesn't mean residents don't deserve solar—it means 
                                they face systemic barriers that policy can address.
                              </p>
                            </div>
                          </>
                        ) : (
                          <div className="text-center py-12">
                            <div className="rounded-3xl border border-orange-400/30 bg-orange-400/10 p-8 text-orange-400">
                              ...
                              <h3 className="text-lg font-semibold mb-2">Equity Analysis Unavailable</h3>
                              <p className="text-sm leading-relaxed">
                                Equity analysis is currently available for Princeton, NJ area only. 
                                We're working to expand coverage to more regions.
                              </p>
                            </div>
                          </div>
                        )}
                      ...
                      >
                        ...
                        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="text-xl font-semibold">Local Solar Installers</h3>
                              <p className="text-sm text-white/70">Hand-picked teams with premium workmanship</p>
                            </div>
                          </div>
                          <p className="mt-4 text-sm text-white/70">
                            We surface certified installers, their specialties, and contact info so you can start quoting immediately.
                          </p>
                          <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-5 backdrop-blur-2xl min-h-[360px]">
                            <div className="prose prose-sm prose-invert max-w-none prose-headings:text-white prose-a:text-white prose-p:text-white prose-li:text-white prose-strong:text-white break-words">
                              {installersContent ? (
                                ...
                                <p className="text-white/60">Tap "Next Steps" to load installer insights.</p>
                              )}
                            </div>
                            {isLoadingInstallers && (
                              <motion.span
                        ...
                        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="text-xl font-semibold">Available Incentives</h3>
                              <p className="text-sm text-white/70">Federal, state, and utility programs curated for you</p>
                            </div>
                          </div>
                          <p className="mt-4 text-sm text-white/70">
                            Unlock stacked rebates, ITC credits, and niche utility perks tailored to this property’s ZIP code.
                          </p>
                          <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-5 backdrop-blur-2xl min-h-[360px]">
                            <div className="prose prose-sm prose-invert max-w-none prose-headings:text-white prose-a:text-white prose-p:text-white prose-li:text-white prose-strong:text-white break-words">
                              {incentivesContent ? (
                                ...
                                <p className="text-white/60">Tap "Next Steps" to load incentive information.</p>
                              )}
                            </div>
                            {isLoadingIncentives && (
                              <motion.span
                  ...
                  <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-3xl border-2 border-amber-400/50 bg-gradient-to-br from-amber-400 via-orange-500 to-amber-600 p-6 shadow-[0_40px_100px_rgba(251,191,36,0.6)]">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-lg font-bold text-slate-900">Score Breakdown</h4>
                      </div>
                      <button
                        onClick={() => setShowScoreBreakdown(false)}
                    ...
                    {stats.solarScoreBreakdown ? (
                      <>
                        <div className="space-y-2.5 mb-4">
                          {Object.entries(stats.solarScoreBreakdown.components).map(([key, component]) => (
                            <div key={key} className="rounded-xl bg-slate-900/20 p-3 backdrop-blur-sm border border-slate-900/10">
                              ...
                              <p className="text-xs text-slate-800 mb-2">{component.details}</p>
                              <div className="flex items-center gap-2 text-xs text-slate-800">
                                <span>Score: {component.score.toFixed(1)}</span>
                                <span>×</span>
                                <span>Weight: {component.weight.toFixed(2)}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="pt-3 border-t-2 border-slate-900/20">
                          <p className="text-xs font-semibold text-slate-900 mb-2">Calculation Formula:</p>
                      ...
                      <div className="text-center py-8">
                        <p className="text-base font-semibold text-slate-900 mb-2">Breakdown data not available</p>
                        <p className="text-sm text-slate-800">The backend server may not be running or this analysis was done before the breakdown feature was added.</p>
                      </div>
                    )}
                  </div>
                </motion.div>
...
const StatCard = ({ icon, label, value, color }: StatCardProps) => {
  const colorClasses = {
    blue: { gradient: "from-sky-500/40 via-sky-400/10 to-transparent", icon: "bg-sky-500/20 text-sky-50" },
    purple: { gradient: "from-purple-500/40 via-fuchsia-500/10 to-transparent", icon: "bg-purple-500/20 text-purple-50" },
    green: { gradient: "from-emerald-500/40 via-emerald-400/10 to-transparent", icon: "bg-emerald-500/20 text-emerald-50" },
    orange: { gradient: "from-amber-500/40 via-amber-400/10 to-transparent", icon: "bg-amber-500/20 text-amber-50" },
  } as const;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col gap-3">
        <div className={`inline-flex h-10 w-10 items-center justify-center rounded-xl ${styles.icon}`}>{icon}</div>
