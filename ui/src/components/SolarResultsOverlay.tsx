...
export const SolarResultsOverlay = ({
    ...
    const buildFinancialOutlook = (solarStats: SolarStats) => {
        ...
        return {
            ...
            net_profit_25_years: Math.round(
                totalSavings - solarStats.systemCost,
            ),
    ...
    const buildSolarPotentialPayload = (solarStats: SolarStats) => {
        ...
        if (solarPotential) {
            return {
                ...
                annual_generation_kwh:
                    solarPotential.annual_generation_kwh ??
                    solarStats.yearlyEnergyKwh,
                daily_generation_kwh:
                    solarPotential.daily_generation_kwh ??
                    solarStats.dailyEnergyKwh,
                capacity_factor:
                    solarPotential.capacity_factor ??
                    Number(
                        (
                            solarStats.yearlyEnergyKwh /
                            (solarStats.systemSizeKw * 8760 || 1)
                        ).toFixed(2),
                    ),
                ...
                equivalent_trees_planted:
                    solarPotential.equivalent_trees_planted ??
                    solarStats.treesEquivalent,
            };
        }
        ...
        const co2TonsYearly = Number(
            (solarStats.carbonOffset / 1000).toFixed(2),
        );
    ...
    const triggerSummaryRefresh = () =>
        setSummaryRequestKey((prev) => prev + 1);
                                            ...
                                            >
                                                ...
                                                <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                                                    ...
                                                    <p className="mt-4 text-sm text-white/70">
                                                        We surface certified
                                                        installers, their
                                                        specialties, and contact
                                                        info so you can start
                                                        quoting immediately.
                                                    </p>
                                                ...
                                                <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-[0_25px_60px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
                                                    ...
                                                    <p className="mt-4 text-sm text-white/70">
                                                        Unlock stacked rebates,
                                                        ITC credits, and niche
                                                        utility perks tailored
                                                        to this property’s ZIP
                                                        code.
                                                    </p>
                                                    <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-5 backdrop-blur-2xl min-h-[360px]">
                                                        <div className="prose prose-sm prose-invert max-w-none prose-headings:text-white prose-a:text-white prose-p:text-white prose-li:text-white prose-strong:text-white break-words">
                                        ...
                                        {stats.solarScoreBreakdown ? (
                                            ...
                                            <div className="text-center py-8">
                                                ...
                                                <p className="text-sm text-slate-800">
                                                    The backend server may not
                                                    be running or this analysis
                                                    was done before the
                                                    breakdown feature was added.
                                                </p>
                                            </div>
                                        )}
                                    </div>
